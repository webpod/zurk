...
var getError = (spawnResult) => {
  if (spawnResult.error)
    return new Error((0, import_error.formatErrorMessage)(spawnResult.error, spawnResult.ctx.stack));
  if (spawnResult.status || spawnResult.signal)
    return new Error((0, import_error.formatExitMessage)(spawnResult.status, spawnResult.signal, spawnResult.stderr, spawnResult.ctx.stack));
  return null;
};
